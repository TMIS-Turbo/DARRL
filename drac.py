...
class Agent():
    ...
    def __init__(self,
        ...
        # Main network
        self.actor = ActorNet(self.obs_dim, self.act_dim, min_log_std=-10.0, max_log_std=5.0).to(device)
    ...
    def train_model(self):
        ...
        if 0:  # Check shape of prediction and target
            ...
            print("cf", cf.shape)
            print("c_backup", c_backup.shape)
            print("cost_pert", cost_pert.shape)
            print("act_err_norm", act_err_norm.shape)
        ...
        cost_pert = self.attacker_constraint(obs1)
        actor_loss = (self.beta * cost_pert - min_q_pi).mean()
        ...
        beta_loss = (self.log_beta * (self.target_cost - cost_pert).detach()).mean()
        ...
        if self.train_times % 100 == 0:
            print("$>>>self.beta<<<$", torch.mean(self.beta))
    ...
    def attacker_constraint(self, state_batch, k=0.2, epsilon=0.2):
        # combined attack
        self.state_batch = state_batch
        target_list_grads = []
        perturb_list = []

        pbounds = {'pert1': (0.8, 1.2)}
        ...
        cost_pert = 0.5*(self.cf1(pert1*state_batch + pert2, pi_pert).squeeze(1).to(device) + self.cf2(pert1*state_batch + pert2, pi_pert).squeeze(1).to(device))
        ...
        mu_pert, std_pert, pi_pert = self.actor(pert1*state_batch + pert2)
        cost_pert = 0.5*(self.cf1(pert1*state_batch + pert2, pi_pert).squeeze(1).to(device) + self.cf2(pert1*state_batch + pert2, pi_pert).squeeze(1).to(device))
        # print("***cost_pert, act_err_norm***", cost_pert.mean())

        return cost_pert

    def cost_pert_black_attack(self, pert1):
        mu_pert, std_pert, pi_pert = self.actor(pert1*self.state_batch)
        cost_pert = 0.5*(self.cf1(pert1*self.state_batch, pi_pert).squeeze(1).to(device) + self.cf2(pert1*self.state_batch, pi_pert).squeeze(1).to(device))
